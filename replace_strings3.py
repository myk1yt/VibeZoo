...
def multi_replace(fpath):
    ...
    content = content.replace("vscode.window.showErrorMessage(`Rewind 실패: ${err.message}`);", "vscode.window.showErrorMessage(vscode.l10n.t('Rewind failed: {0}', err.message));")
    
    content = content.replace("vscode.window.showErrorMessage(`❌ Crow 연결 실패: ${err.message}`);", "vscode.window.showErrorMessage(vscode.l10n.t('❌ Crow connection failed: {0}', err.message));")
    ...
    content = content.replace("this.item.tooltip = `VibeZoo: ${reason}\\n클릭하여 모드 변경`;", "this.item.tooltip = vscode.l10n.t('VibeZoo: {0}\\nClick to change mode', reason);")

    content = content.replace("this.tooltip = `YOLO 세션: ${name}\\n우클릭 → Rewind 실행`;", "this.tooltip = vscode.l10n.t('YOLO Session: {0}\\nRight-click → Run Rewind', name);")

    content = content.replace("vscode.window.showInformationMessage(`✅ ${fileTypeLabel}이 업로드되었습니다. (경로가 클립보드에 복사되었습니다. 채팅창에 붙여넣어 LLM에게 지시하세요!)`);", "vscode.window.showInformationMessage(vscode.l10n.t('✅ {0} uploaded. (Path copied to clipboard. Paste it in chat to instruct the LLM!)', fileTypeLabel));")
