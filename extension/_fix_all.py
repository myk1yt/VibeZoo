...
# Add openDropzone(), handleImageUpload(), openFilePicker() after openWhiteboard()
...
with open(ts_path, 'r', encoding='utf-8') as f:
    v = f.read()
print('DROPZONE_CACHE_DIR:', 'DROPZONE_CACHE_DIR' in v)
print('dropzonePanel prop:', 'private dropzonePanel' in v)
print('openDropzone:', 'openDropzone(htmlB64' in v)
print('handleImageUpload:', 'handleImageUpload' in v)
print('openFilePicker:', 'openFilePicker' in v)
print('open_dropzone action:', \"'open_dropzone'\" in v)
print('open_file_picker action:', \"'open_file_picker'\" in v)

# ── Fix 2: whiteboard.py _DROPZONE_HTML ──
...
if old_js in wb:
    ...
    with open(wb_path, 'r', encoding='utf-8') as f:
        wv = f.read()
    print('whiteboard acquireVsCodeApi:', 'acquireVsCodeApi' in wv)
    print('whiteboard old fetch:', 'fetch(' in wv and '/upload' in wv)
else:
    print('whiteboard: old_js NOT FOUND, checking...')
    idx = wb.find('<script>')
    if idx >= 0:
        print('Found <script> at', idx, 'context:', repr(wb[idx:idx+80]))
