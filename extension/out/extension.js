...
const FileGuard_1 = require("./safety/FileGuard");
...
(function trySpawnEarlyBridge() {
    try {
        ...
        const child = (0, child_process_1.spawn)('python', [scriptPath, '--port', '9027'], {
            detached: true,
...
async function activate(context) {
    ...
    if (vscode.workspace.getConfiguration('vibezoo').get('yolo.enabled', true)) {
        ...
        fileGuard = new FileGuard_1.FileGuard(yocto);
    ...
    context.subscriptions.push(vscode.commands.registerCommand('vibezoo.instantRewind', async (sessionName) => {
        ...
        try {
            ...
            vscode.window.showInformationMessage(`YOLO Rewind 완료: ${result.restoredFiles}/${result.totalFiles} 파일 복구 (${result.durationMs}ms)`);
        }
        catch (err) {
    ...
    if (!hasShownWelcome) {
        ...
        setTimeout(() => {
            vscode.window.showInformationMessage('🎉 VibeZoo 준비 완료! Ctrl+Shift+P → VibeZoo: Help', 'Help 보기', '닫기').then(choice => {
                if (choice === 'Help 보기')
    ...
    // FileGuard 토글 커맨드
    context.subscriptions.push(vscode.commands.registerCommand('vibezoo.toggleFileGuard', async () => {
        if (!fileGuard) {
...
function autoConfigureMCP(port = 9027) {
