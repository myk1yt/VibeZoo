...
class SubagentManager {
    ...
    getBridgePort() {
        return ConfigService_1.ConfigService.getBridgePort();
    }
    /** 개별 에이전트 포트 목록 */
    getAgentPorts() {
        return ConfigService_1.ConfigService.getAgentPorts();
    }
    ...
    async spawnBridge() {
        ...
        this.child = (0, child_process_1.spawn)('python', [this.bridgeScript, '--port', String(port)], {
            ...
            env: {
                ...process.env,
                CROW_SERVER_URL: ConfigService_1.ConfigService.getCrowUrl(),
            },
        });
        this.child.unref();
                ...
                if (this.child && this.child.pid) {
                    try {
                        ...
                        if (process.platform === 'win32') {
                            (0, child_process_1.execSync)(`taskkill /F /PID ${this.child.pid} /T`, { timeout: 3000 });
                        }
                        else {
    ...
    async checkHealth(port) {
        try {
            ...
            const response = await fetch(ConfigService_1.ConfigService.getAgentUrl(port, '/health'), {
        ...
        if (this.child) {
            ...
            try {
                if (process.platform === 'win32' && pid) {
                    (0, child_process_1.execSync)(`taskkill /F /PID ${pid} /T`, { timeout: 3000 });
                }
                else {
                    child.kill('SIGTERM');
                    setTimeout(() => {
