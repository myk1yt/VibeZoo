...
class YoctoManager {
    ...
    activate(context) {
        ...
        if (!folders || folders.length === 0)
            return;
        // 소스 파일 변경 감시
        this.watcher = vscode.workspace.createFileSystemWatcher(new vscode.RelativePattern(folders[0], '**/*.{ts,tsx,js,jsx,py,go,rs,java,json,yaml,yml,md,css,html}'), false, // 생성
        false, // 변경
        false // 삭제
        );
