...
class FileGuard {
    ...
    _recentlyRestored = new Map();
    RESTORE_COOLDOWN_MS = 5000; // 5초 이내 동일 파일 복구 → 무시
    /** FileGuard ON/OFF 상태 */
    _enabled = true;
    constructor(yocto) {
        this.yocto = yocto;
    ...
    activate(context) {
        ...
        const ignoreWatcher = vscode.workspace.createFileSystemWatcher(new vscode.RelativePattern(folders[0], '.yoloignore'));
        ...
        // 모든 파일 변경 감시
        this.watcher = vscode.workspace.createFileSystemWatcher(new vscode.RelativePattern(folders[0], '**/*'), false, false, false);
        ...
        for (const source of sources) {
            try {
                ...
                for (const line of lines) {
                    this.patterns.push(new minimatch_1.Minimatch(line.trim(), { dot: true }));
                }
            }
            catch {
                // 파일 없음
    ...
    findLatestBackup(originalPath) {
        const yoctoDir = path.join(os.homedir(), '.zoo-code', 'yocto');
        if (!fs.existsSync(yoctoDir))
            return null;
        const relativePath = path.relative(vscode.workspace.workspaceFolders?.[0]?.uri.fsPath ?? '', originalPath);
        // 세션 디렉토리 목록 (역순 정렬로 최신 세션 우선)
        const sessions = fs.readdirSync(yoctoDir)
            .filter((s) => {
            try {
                return fs.statSync(path.join(yoctoDir, s)).isDirectory();
            }
            catch {
                return false;
            }
        ...
        for (const session of sessions) {
            ...
            const timestamps = fs.readdirSync(sessionDir)
                .filter((t) => {
                try {
                    return fs.statSync(path.join(sessionDir, t)).isDirectory();
                }
                catch {
                    return false;
                }
