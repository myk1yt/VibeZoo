...
exports.StatusBarManager = exports.NotificationThrottle = void 0;
const vscode = __importStar(require("vscode"));
// ── NotificationThrottle ──────────────────────────────────────
class NotificationThrottle {
    static _history = new Map();
    static _minuteCount = [];
    static SAME_MSG_WINDOW_MS = 3000; // 동일 메시지 3초 내 중복 → 무시
    static MAX_PER_MINUTE = 10; // 분당 최대 10회
    /**
     * 메시지가 throttle 조건을 통과하면 true 반환.
     * @param message 표시할 메시지
     * @param useStatusBarFallapthrottle 초과 시 StatusBar로 대체 표시
     */
    static shouldAllow(message) {
        const now = Date.now();
...
class StatusBarManager {
    item;
    modeSuggestionTimer = null;
    savedText = '';
    savedTooltip = '';
    savedCommand = '';
    _crowConnected = false;
    _cimActive = false;
    _yoloActive = false;
    _guardMode = 'safe';
    /** setActive()로 설정된 base tooltip (Crow 접미사 제외) */
    _baseTooltip = vscode.l10n.t('VibeZoo: Active');
    constructor() {
    ...
    _composeText() {
        if (this._guardMode === 'active')
            return '$(zap) VibeZoo Guard';
        if (this._guardMode === 'warning')
            return '$(warning) VibeZoo';
    ...
    /** VibeZoo 활성 상태 표시 */
    setActive(bridgeConnected, bridgePort, crowConnected) {
    ...
    /** 권장 모드 제안 (5초 후 자동 복구) */
    suggestMode(mode, reason) {
        if (this.modeSuggestionTimer)
            clearTimeout(this.modeSuggestionTimer);
