...
from bridge.embedding_client import EmbeddingClient, rank_by_embedding, _get_embed_client
...
def _search_codebase_impl(query: str, file_patterns: Optional[str] = None,
    ...
    if mode == "fuzzy":
        ...
        tokens = [tok for tok in broadened_query.split() if len(tok) >= 2]
        if tokens:
    ...
    if mode == "semantic" and search_results:
        ...
        if embed_client.is_available():
            ...
            if query_vec and query_vec[0]:
                ...
                emb_ranking_msg = t('semantic: embedding-based ranking (rank_source="embedding")')
                semantic_note = f"> 🧠 {emb_ranking_msg}\n\n"
            else:
                ranker = ResultRanker()
            ...
            semantic_note = (
                f"> ⚠️ {t('semantic: embedding server unavailable, used BM25 keyword ranking')}\n"
                f"> 💡 {down_guidance}\n\n"
            )
...
def _find_references_impl(symbol: str, target_path: Optional[str] = None) -> str:
    ...
    if usages:
        ...
        for tool_type, label in type_labels.items():
            items = ref_types.get(tool_type, [])
            if items:
...
def _embedding_health_check_impl() -> str:
    """Check embedding server health and return status dict as JSON."""
    client = _get_embed_client()
    available = client.is_available()
    api_style = getattr(client, "api_style", None) or getattr(client, "_api_style", None) or "unknown"
    url = getattr(client, "base_url", None) or getattr(client, "_base_url", "http://localhost:8089")
    model = getattr(client, "model_name", None) or getattr(client, "_model", "nomic-embed-text")
    hint = "" if available else t(
        "Cannot connect to embedding server (localhost:8089, nomic-embed-text). Recovery: [1] Load nomic-embed-text model in LM Studio/Ollama and run on port 8089. [2] Retry the same request after server starts (auto-reprobes). Manual code index rebuild: VS Code command 'VibeZoo: Rebuild Code Index'."
    )
    result = {
        "available": available,
...
def _rebuild_code_index_impl(target_path: Optional[str] = None) -> str:
    """Rebuild disk-backed code index cache for workspace."""
    root = Path(get_project_root(target_path)) if target_path else Path(os.getcwd())
    client = _get_embed_client()
    try:
        from bridge.index_cache import CodeIndexCache
        cache = CodeIndexCache(workspace_root=root)
    except Exception as e:
        return _markdown_header(t("Code Index Rebuild"), "❌") + f"**{e}**\n" + _markdown_footer()

    if not client.is_available():
        guidance = t(
            "Cannot connect to embedding server (localhost:8089, nomic-embed-text). Recovery: [1] Load nomic-embed-text model in LM Studio/Ollama and run on port 8089. [2] Retry the same request after server starts (auto-reprobes). Manual code index rebuild: VS Code command 'VibeZoo: Rebuild Code Index'."
        )
        return _markdown_header(t("Code Index Rebuild"), "⚠️") + f"**{guidance}**\n" + _markdown_footer()

    model = getattr(client, "model_name", None) or getattr(client, "_model", "nomic-embed-text")
    count = cache.rebuild(root=root, embed_fn=client.embed, model_name=model)
    msg = t("Code index rebuilt successfully ({0} files indexed).", count)
    return _markdown_header(t("Code Index Rebuild"), "✅") + f"**{msg}**\n" + _markdown_footer()
...
def register(mcp):
    ...
    @mcp.tool
    def embedding_health_check() -> str:
        """Embedding 서버(localhost:8089) 상태 및 가용성을 점검합니다.

        Returns:
            JSON string with available, api_style, url, model, hint.
        """
        return _embedding_health_check_impl()

    @mcp.tool
    def rebuild_code_index(target_path: Optional[str] = None) -> str:
        """코드베이스 인덱스 캐시(.zoo-code/index-cache/)를 수동으로 리빌드합니다.

        Args:
            target_path: 대상 디렉토리 경로 (기본: 현재 워크스페이스)
        """
        return _rebuild_code_index_impl(target_path)
