...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  ...
  // K-2. vibezoo.rebuildCodeIndex — 코드 인덱스 재구축 (MCP 툴 안내)
  context.subscriptions.push(
    vscode.commands.registerCommand('vibezoo.rebuildCodeIndex', async () => {
      vscode.window.showInformationMessage(
        vscode.l10n.t('VibeZoo: Please type "rebuild code index" in Zoo Code chat. (rebuild_code_index MCP tool)')
      );
    })
  );
