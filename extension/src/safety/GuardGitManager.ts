...
import { IGuardGitACL, createGuardGitACL } from './GuardGitACL';
...
export class GuardGitManager {
  // ── 상태 ──
  private stateMap: Map<string, GuardGitState> = new Map();
  private gitDirPaths: string[] = [];
  private acl: IGuardGitACL;
  private watchers: Map<string, vscode.FileSystemWatcher> = new Map();
  private yocto: YoctoManager | null = null;
  private statusBar: StatusBarManager | null = null;
  private selfCheckInterval: NodeJS.Timeout | null = null;
  private yoctoBackupInterval: NodeJS.Timeout | null = null;
  ...
  private _onChangeCallbacks: Array<(
    summary: { overall: GuardGitState; paths: Map<string, GuardGitState> }
  ) => void> = [];

  constructor() {
    this.acl = createGuardGitACL();
  }
  ...
  private rescanWorkspaceFolders(): void {
  ...
  async enable(): Promise<GuardGitACLResult> {
    ...
    let allSuccess = true;
    let lastResult: GuardGitACLResult = { success: true };
    ...
    for (const gitDir of this.gitDirPaths) {
      ...
      if (process.platform === 'linux' && !ConfigService.getGuardLinuxUseChattr()) {
    ...
    this.createGitSnapshot('guard-enable').catch(err =>
      console.warn('[Guard.git] Yocto 스냅샷 실패:', err)
    );

    // 4. 주기적 진단 시작 (H5)
    const intervalMin = ConfigService.getGuardIntegrityCheckIntervalMin();
    this.startPeriodicIntegrityCheck(intervalMin * 60 * 1000);

    // 5. Yocto 백업 시작
    if (ConfigService.getGuardYoctoBackupEnabled()) {
      const backupIntervalMin = ConfigService.getGuardYoctoBackupIntervalMin();
   ...
   */
  async disable(): Promise<GuardGitACLResult> {
    let allSuccess = true;
    let lastResult: GuardGitACLResult = { success: true };
  ...
  isEnabled(): boolean {
    for (const state of this.stateMap.values()) {
      if (state === 'active' || state === 'warning') return true;
    }
    return false;
  }
   ...
   * C4: 모든 경로 순회
   */
  async checkIntegrity(): Promise<GuardGitIntegrity[]> {
    const results: GuardGitIntegrity[] = [];
  ...
  private async checkSingleIntegrity(gitDir: string): Promise<GuardGitIntegrity> {
    const base: GuardGitIntegrity = {
      exists: false,
      protected: false,
      headRef: null,
      objectCount: 0,
  ...
  private startYoctoBackup(intervalMs: number): void {
    this.stopYoctoBackup();
    this.yoctoBackupInterval = setInterval(async () => {
      await this.createGitSnapshot('guard-periodic').catch(err =>
        console.warn('[Guard.git] 주기적 Yocto 백업 실패:', err)
      );
    }, intervalMs);
  }

  private stopYoctoBackup(): void {
   ...
   */
  async createGitSnapshot(
    trigger: 'guard-enable' | 'guard-periodic' | 'guard-pre-danger'
  ): Promise<void> {
    if (!this.yocto) {
   ...
   */
  onChange(
    cb: (summary: { overall: GuardGitState; paths: Map<string, GuardGitState> }) => void
  ): () => void {
  ...
  private computeOverallState(): GuardGitState {
    let hasActive = false;
  ...
  shouldAutoEnable(): boolean {
    if (!ConfigService.getGuardAutoEnable()) return false;
