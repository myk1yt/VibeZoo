...
export class AlarmMonitor {
  private alarmLog: { timestamp: number; message: string }[] = [];
  private readonly WINDOW_MS = 60_000; // 60초 슬라이딩 윈도우
  private readonly MAX_ALARMS = 30;     // 분당 30회
  ...
  record(message: string): boolean {
    ...
    if (this.alarmLog.length > this.MAX_ALARMS) {
      ...
      console.error(
        `[AlarmMonitor] CRITICAL: 분당 ${this.alarmLog.length}회 알람 발생! 30초간 throttle. 마지막 메시지: ${message}`
      );
      return true; // 무시됨
    }
...
export class SelfChecker {
  ...
  /** 모든 진단 실행 */
  async runAll(): Promise<SelfCheckReport> {
    ...
    let overall: SelfCheckReport['overall'] = 'healthy';
    if (failures > 0) overall = 'critical';
  ...
  /** Crow :9020/health 확인 */
  async checkCrowHealth(): Promise<SelfCheckItem> {
    const base: SelfCheckItem = {
  ...
  /** 화이트보드 JSON 파일 무결성 */
  async checkWhiteboardFiles(): Promise<SelfCheckItem> {
    const base: SelfCheckItem = {
  ...
  /** 백업 디렉토리 권한/공간 확인 */
  async checkYoctoDirectory(): Promise<SelfCheckItem> {
    const base: SelfCheckItem = {
  ...
  /** Zoo Code 확장 호환성 확인 */
  async checkZooCodeCompatibility(): Promise<SelfCheckItem> {
    const base: SelfCheckItem = {
  ...
  /** 알람 카운터 헬스 체크 */
  async checkNotificationHealth(): Promise<SelfCheckItem> {
    const base: SelfCheckItem = {
    ...
    try {
      const recentCount = alarmMonitor.recentAlarmCount;
      const isThrottled = alarmMonitor.throttled;
  ...
  /** 감지된 문제 자동 복구 시도 */
  async autoRecover(failure: SelfCheckItem): Promise<boolean> {
    if (!failure.autoRecoverable) return false;
  ...
  formatReport(report: SelfCheckReport): string {
    ...
    for (const check of report.checks) {
      ...
      if (check.autoRecoverable) lines.push(`**Auto-Recoverable**: Yes`);
      lines.push('');
    }
    ...
    if (failed > 0 || warnings > 0) {
      ...
      lines.push('');
      if (failed > 0) lines.push(`- ❌ ${failed}개 실패 항목 — 자동 복구 가능한 항목은 "VibeZoo: Self Check" 명령어로 재시도`);
      if (warnings > 0) lines.push(`- ⚠️ ${warnings}개 경고 — 필요 시 수동 확인`);
      lines.push('');
    }
