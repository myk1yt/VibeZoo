...
> **VibeZoo = [Crow Memory](#crow-memory-overview) (Synaptic Memory) + [VibeZoo MCP Bridge](#mcp-tool-catalog) (40 Tools)**
>
...
| **Key Features** | Guard.git (`.git` protection), YOLO snapshots, Whiteboard/Dropzone, Crow Memory integration, 40 MCP tools |
| **License** | MIT |
...
│ • Real Crow (proxy mode)    │  │ • 40 MCP Tools                  │
...
| **Layer 2** | [`mcp-servers/bridge/`](mcp-servers/bridge/) | Python MCP Bridge — 40 tools, AST, search, OCR, vision, error handling |
...
| **Tools** | [`tools/`](mcp-servers/bridge/tools/__init__.py:1) | 16 files, 40 MCP tools registered |
...
The VibeZoo MCP Bridge provides 40 MCP tools modularized across 16 files.
...
| Integrated | [`tools/integrated.py`](mcp-servers/bridge/tools/integrated.py) | `review_project`, `find_bugs`, `suggest_refactor`, `generate_docs` | Unified scenario tools |
...
| Knowledge | [`tools/knowledge.py`](mcp-servers/bridge/tools/knowledge.py) | `learn_project`, `recall_project`, `learn_preference`, `get_preferences` | Project/user knowledge memory |
...
| Editor | [`tools/editor.py`](mcp-servers/bridge/tools/editor.py) | `apply_patch`, `read_project_file` | AI-safe file editing |
| UX Coordinator | [`tools/ux_coordinator.py`](mcp-servers/bridge/tools/ux_coordinator.py) | `ux_coordinator`, `auto_analyze_after_drop` | Intent detection and auto tool chains |
| Feedback | [`tools/feedback.py`](mcp-servers/bridge/tools/feedback.py) | `vibezoo_feedback` | Feedback/telemetry logging |

> **v0.16.0 Note**: `auto_analyze_whiteboard` has been merged into `get_whiteboard_state(analyze=True)`. The old name remains as a deprecated alias.
...
| 2 | **Python Bridge SPOF** | 40 MCP tools run in a single Python process (`vibezoo_mcp_bridge.py`). If it crashes, all MCP features go down. | [`mcp-servers/vibezoo_mcp_bridge.py`](mcp-servers/vibezoo_mcp_bridge.py:1) |
